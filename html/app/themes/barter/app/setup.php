...
add_action ('template_redirect', function() {
    if (is_page('dashboard')) {
        if(!is_user_logged_in()){
            $redirect = home_url('/ingreso');
            wp_safe_redirect($redirect);
            exit;
        }
    } elseif (is_page('ingreso')){
        if(is_user_logged_in()){
...
add_action('wp_ajax_nopriv_update_status_component', function(){
    return view('components/transaction-status', [])->render();
});
...
add_action( 'wp_ajax_nopriv_create_user', function(){
    ...
    if( !is_wp_error($user_id) ) {
        ...
        foreach ($criptomonedas->posts as $c) 
        {
            ...
            update_field("usuario", $user_id, $id);
        }
...
include('Setup/BarterFunctions.php');
